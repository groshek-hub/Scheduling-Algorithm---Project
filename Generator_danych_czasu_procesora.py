...
def generator():
    ...
    for x in range(
        ...
        wartosci.write(str(random.randint(1, 21)) + "\n")  # wypisywane sa wartości od 1-20
    wartosci.close()  # zamykanie pliku


def generator_czas_przybycia():
    wartosci_czas_przybycia = open("wartosci_czas_przybycia.txt",
                                   "w")  # plik otwarty do zapisu (write), przed zapisem zawartość pliku jest usuwana
    for x in range(
            10000):  # range()generuje liczby całkowite między podaną liczbą całkowitą początkową a liczbą całkowitą
        # zatrzymania , tj. Zwraca obiekt zakresu(10000).
        wartosci_czas_przybycia.write(str(random.randint(0,
                                                         101)) + "\n")  # wypisywane sa wartości od 0-100,
        # random.randint() zwraca losowy element całkowity wybrany z danego przedziału
    wartosci_czas_przybycia.close()  # zamykanie pliku


def generator_czas_trwania():
    wartosci = open("wartosci.txt", "w")  # plik otwarty do zapisu (write), przed zapisem zawartość pliku jest usuwana
    for x in range(
            10000):  # range()generuje liczby całkowite między podaną liczbą całkowitą początkową a liczbą całkowitą
        # zatrzymania , tj. Zwraca obiekt zakresu(10000).
...
# FUNCKJE
generator_czas_trwania()  # wywołanie funkcji
generator_czas_przybycia()  # wywołanie funkcji
