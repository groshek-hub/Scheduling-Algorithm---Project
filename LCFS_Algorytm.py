...
def revfun(lst):  # funkcja do odwracania elementów listy
    lst.reverse()  # Metoda reverse () odwraca elementy listy
    return lst  # zwraca odwrócono już listę


# W poniższej klasie zawieramy informacje o dwóch wartościach
def funkcja_LCFS():
    class Proces:  # klasa z funkcjami
        def __init__(self, czas_przybycia, czas_trwania):  # __init__ metoda przypisana w klasach Pythona
            ...
            self.czas_trwania = czas_trwania  # czas_trwania (i jest on z zakresu od 1 do 20)
            self.czas_przybycia = czas_przybycia  # czas_przybycia (jest on z zakresu od 0 do 100)
    ...
    for z in range(100):
        ...
        for i in range(101):
            for j in range(100):  # dodajemy procesy do kolejki j by osiągnąć = 101, ponieważ kolejka musi mieć 100
                ...
                if i >= tablica_100[j].czas_przybycia and (tablica_100[j] not in kolejka):
                    kolejka.append(tablica_100[j])

        revfun(kolejka)  # wywołanie funkcji odwracającą listę(kolejka)

        for j in range(1, 100):
            for m in range(0, j):
                czas_oczekiwania[j] += kolejka[m].czas_przybycia - kolejka[m + 1].czas_przybycia + kolejka[
                    m].czas_trwania
        ...
        for j in range(100):
            ...
            sr_czas_wykonywania += czas_wykonywania[j]  # wykonane dla 100 prób, inkrementacja
            zsr_czas_wykonywania += czas_wykonywania[j]  # wykonane dla pojedyńczej próby, inkrementacja
    ...
    del tablica_100[:], kolejka[:]  # usunięcie wszystkich elementów z listy tablica_100 oraz kolejka
...
funkcja_LCFS()  # wywołanie funkcji
